...
import { closeSettingsDialog, setActiveSettingsSection } from '../model/slices'
import { SettingsContent } from './SettingsContent'
import { SettingsSidebar } from './SettingsSidebar'

export function SettingsDialog(): React.JSX.Element | null {
  const dispatch = useAppDispatch()
  const isOpen = useAppSelector(selectIsSettingsDialogOpen)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6">
      <div
        ...
        aria-modal="true"
        aria-label="设置"
        className="flex h-[720px] w-full max-w-[970px] overflow-hidden rounded-2xl border border-[#3a414f] bg-[#2b313c] text-[#eef2f7] shadow-[0_32px_80px_rgba(0,0,0,0.45)]"
      >
        <SettingsSidebar
        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-[60px] items-center justify-between border-b border-[#3a414f] px-6">
          ...
          <footer className="flex items-center justify-between border-t border-[#3a414f] px-6 py-4">
